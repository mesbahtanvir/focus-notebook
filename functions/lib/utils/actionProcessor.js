...
exports.buildThoughtUpdate = buildThoughtUpdate;
exports.countChanges = countChanges;
const config_1 = require("../config");
/**
 * Process actions into auto-apply and suggestions
 */
function processActions(actions, currentThought) {
    const result = {
        autoApply: {
            tagsToAdd: [],
        },
    ...
    for (const action of actions) {
        ...
        if (action.confidence >= config_1.CONFIG.CONFIDENCE_THRESHOLDS.AUTO_APPLY) {
            applyHighConfidenceAction(action, result, currentThought);
        }
        // Store medium confidence as suggestions
        else if (action.confidence >= config_1.CONFIG.CONFIDENCE_THRESHOLDS.SUGGEST) {
 ...
 */
function applyHighConfidenceAction(action, result, currentThought) {
    var _a, _b, _c, _d;
    switch (action.type) {
        case 'enhanceThought':
        ...
        case 'addTag':
            ...
            if (!((_a = currentThought.tags) === null || _a === void 0 ? void 0 : _a.includes(tag)) && !result.autoApply.tagsToAdd.includes(tag)) {
        ...
        case 'linkToGoal':
            ...
            if (!((_b = currentThought.tags) === null || _b === void 0 ? void 0 : _b.includes(goalTag)) && !result.autoApply.tagsToAdd.includes(goalTag)) {
                result.autoApply.tagsToAdd.push(goalTag);
            }
            break;
        case 'linkToProject':
            const projectTag = `project-${action.data.projectId}`;
            if (!((_c = currentThought.tags) === null || _c === void 0 ? void 0 : _c.includes(projectTag)) && !result.autoApply.tagsToAdd.includes(projectTag)) {
                result.autoApply.tagsToAdd.push(projectTag);
            }
            break;
        case 'linkToPerson':
            const personTag = `person-${action.data.shortName}`;
            if (!((_d = currentThought.tags) === null || _d === void 0 ? void 0 : _d.includes(personTag)) && !result.autoApply.tagsToAdd.includes(personTag)) {
 ...
 */
function buildThoughtUpdate(processedActions, currentThought, tokensUsed, trigger) {
 ...
 */
function countChanges(processedActions) {
    let count = 0;
    if (processedActions.autoApply.text)
        count++;
