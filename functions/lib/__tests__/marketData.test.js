...
describe('marketData helpers', () => {
    ...
    describe('delay', () => {
        ...
        it('resolves after the specified duration', async () => {
            ...
            const promise = marketData_1.__private__.delay(500).then(callback);
    ...
    describe('fetchTickerQuote', () => {
        ...
        it('fetches and parses a ticker quote', async () => {
            ...
            const result = await marketData_1.__private__.fetchTickerQuote('AAPL', 'demo-key');
        ...
        it('throws when the provider returns a non-OK response', async () => {
            ...
            global.fetch = fetchMock;
            await expect(marketData_1.__private__.fetchTickerQuote('AAPL', 'demo-key')).rejects.toThrow('Rate limited');
        });
    });
    describe('runUpdateTrackedTickers', () => {
        it('aggregates tickers and currencies then persists to Firestore', async () => {
            ...
            const fieldValue = { serverTimestamp: jest.fn(() => 'timestamp') };
            const result = await marketData_1.__private__.runUpdateTrackedTickers({ eventId: 'event-1' }, {
                firestore,
                logger,
                fieldValue,
            });
    ...
    describe('runRefreshTrackedTickerPrices', () => {
        const buildFirestore = () => {
            ...
            const marketDataDoc = jest.fn().mockImplementation((docName) => {
        ...
        it('persists refreshed ticker prices and records failures', async () => {
            ...
            const result = await marketData_1.__private__.runRefreshTrackedTickerPrices({ eventId: 'run-1' }, {
                firestore,
                logger,
                fieldValue,
                apiKey: 'test-key',
            ...
            expect(batchSet).toHaveBeenNthCalledWith(1, latestRef, {
                tickers: {
                    AAPL: {
                        symbol: 'AAPL',
                        price: 170,
            ...
            expect(batchSet).toHaveBeenNthCalledWith(2, historyDoc, expect.objectContaining({
                date: '2024-05-11',
                totalFailures: 1,
            }));
        ...
        it('logs when tracked tickers document is missing', async () => {
            ...
            const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
            const fieldValue = { serverTimestamp: jest.fn(() => 'timestamp') };
            const result = await marketData_1.__private__.runRefreshTrackedTickerPrices({ eventId: 'missing-doc' }, {
                firestore,
                logger,
                fieldValue,
                apiKey: 'test-key',
        ...
        it('logs when API key is missing', async () => {
            ...
            const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
            const fieldValue = { serverTimestamp: jest.fn(() => 'timestamp') };
            const result = await marketData_1.__private__.runRefreshTrackedTickerPrices({ eventId: 'no-key' }, {
                firestore,
                logger,
                fieldValue,
                apiKey: '',
