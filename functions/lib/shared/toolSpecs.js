...
exports.ALL_TOOL_IDS = exports.CORE_TOOL_IDS = exports.toolSpecs = void 0;
exports.getToolSpecById = getToolSpecById;
exports.renderToolSpecForPrompt = renderToolSpecForPrompt;
exports.renderToolSpecsForPrompt = renderToolSpecsForPrompt;
const baseGuidance = [
...
exports.toolSpecs = {
    thoughts: {
        ...
        description: 'General purpose processing for everyday thoughts. Enhance clarity, add helpful tags, and surface actionable follow-ups.',
        ...
        positiveExamples: [
            {
                ...
                rationale: 'Clear next steps and named collaborator. Beneficial to create a follow-up task and tag the project/person.',
                recommendedActions: [
                    {
                        type: 'addTag',
        ...
        negativeExamples: [
            {
                ...
                rationale: 'Too vague to create tasks or tags; avoid guessing intent when no action is implied.',
                recommendedActions: [],
            },
            {
                thought: 'Feeling kind of off today, not sure why.',
                rationale: 'Mood is present but no actionable content; log as mood suggestion instead of project/task.',
                recommendedActions: [
                    {
                        type: 'createMood',
    ...
    tasks: {
        ...
        description: 'Manage todos, priorities, and progress in one place. Ideal for capturing next steps generated from thought processing.',
    ...
    projects: {
        ...
        description: 'Plan, group, and track related tasks or milestones. Ideal when a thought references a broader initiative.',
    ...
    goals: {
        ...
        description: 'Define personal or professional goals, align projects, and measure progress over time.',
    ...
    focus: {
        ...
        description: 'Run structured focus blocks, track streaks, and log outcomes for high-leverage work.',
    ...
    brainstorming: {
        ...
        description: 'Generate ideas, prompts, and new angles by chatting with an AI collaborator.',
    ...
    notes: {
        ...
        description: 'Store evergreen knowledge, meeting notes, and references separated from action-oriented tools.',
    ...
    relationships: {
        ...
        description: 'Maintain relationship histories, energy levels, and follow-up reminders for people that matter.',
    ...
    moodtracker: {
        ...
        description: 'Record daily mood, triggers, and notes to understand emotional trends.',
    ...
    cbt: {
        ...
        description: 'Identify cognitive distortions, capture emotions, and guide the user through reframing negative thinking patterns.',
        ...
        positiveExamples: [
            {
                ...
                rationale: 'Strong negative self-talk and distortion language; prime candidate for CBT analysis.',
                recommendedActions: [
                    {
                        type: 'addTag',
        ...
        negativeExamples: [
            {
                ...
                rationale: 'Task-oriented with no emotional distress; CBT tagging would be inappropriate.',
                recommendedActions: [
                    {
                        type: 'addTag',
    ...
    deepreflect: {
        ...
        description: 'A space for long-form reflection, identity work, and existential exploration.',
    ...
    errands: {
        ...
        description: 'Manage short, location-based, or recurring errands separate from deep work tasks.',
    ...
    'packing-list': {
        ...
        description: 'Create smart packing lists tailored to trip type, destination, and length.',
    ...
    trips: {
        ...
        description: 'Manage trip budgets, itineraries, and shared tasks across your travel plans.',
    ...
    investments: {
        ...
        description: 'Stay on top of portfolio performance, allocation, and investment ideas.',
    ...
    subscriptions: {
        ...
        description: 'Track trials, renewals, and recurring expenses to avoid surprises.',
    ...
    'asset-horizon': {
        ...
        description: 'Model investment growth scenarios, retirement horizons, and savings milestones.',
...
function getToolSpecById(id) {
    const spec = exports.toolSpecs[id];
    if (!spec) {
        throw new Error(`Tool spec "${id}" is not defined`);
    }
    return spec;
}
function renderToolSpecForPrompt(spec) {
    const expected = spec.expectedCapabilities.join(', ');
    const guidance = spec.guidance.map((item, index) => `${index + 1}. ${item}`).join('\n');
    const formatExamples = (examples, label) => examples
        .map((ex, index) => `${label} ${index + 1}\nThought: "${ex.thought}"\nRationale: ${ex.rationale}${ex.recommendedActions.length
        ? `\nSuggested actions: ${ex.recommendedActions
            .map((action) => `${action.type}${action.dataSummary ? ` - ${action.dataSummary}` : ''}${action.confidence ? ` (confidence ${action.confidence})` : ''}`)
            .join('; ')}`
        : ''}`)
        .join('\n\n');
    return [
...
function renderToolSpecsForPrompt(specs) {
    return specs.map(renderToolSpecForPrompt).join('\n\n---\n\n');
}
//# sourceMappingURL=toolSpecs.js.map