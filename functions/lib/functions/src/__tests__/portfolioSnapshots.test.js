...
const collectionGetMap = {};
const ensureCollectionGet = (path) => {
...
const { normalizeCurrency, convertToBaseCurrency, fetchFxRates, createSnapshotForPortfolio, FALLBACK_RATES, } = __private__;
describe('portfolioSnapshots helpers', () => {
    ...
    it('converts amounts to the base currency using provided FX rates', () => {
        const rates = { USD: 1, CAD: 2, BDT: 100, COP: 4000 };
        expect(convertToBaseCurrency(200, 'cad', rates)).toBeCloseTo(100);
        expect(convertToBaseCurrency('500', 'bdt', rates)).toBeCloseTo(5);
        expect(convertToBaseCurrency('not-number', 'cad', rates)).toBe(0);
        expect(convertToBaseCurrency(300, 'usd', rates)).toBe(300);
    });
    const mockSuccessfulFxRequest = (payload) => {
        requestMock.mockImplementationOnce((_url, callback) => {
            ...
            const request = {
                ...
                setTimeout: jest.fn(),
                    ...
                    var _a, _b;
                    (_a = responseHandlers['data']) === null || _a === void 0 ? void 0 : _a.call(responseHandlers, Buffer.from(JSON.stringify(payload)));
                    (_b = responseHandlers['end']) === null || _b === void 0 ? void 0 : _b.call(responseHandlers);
                }),
                destroy: jest.fn(),
            };
            callback(response);
    ...
    it('creates a snapshot document with converted investment values', async () => {
        ...
        await createSnapshotForPortfolio('user-1', 'portfolio-1', portfolio, rates);
        expect(docMock).toHaveBeenCalledWith('users/user-1/portfolios/portfolio-1/snapshots/daily-2024-05-15');
    ...
    it('runs the scheduled job and processes portfolios', async () => {
        ...
        docSetMock.mockResolvedValue(undefined);
        await (capturedOnRun === null || capturedOnRun === void 0 ? void 0 : capturedOnRun());
        expect(requestMock).toHaveBeenCalledTimes(1);
        expect(docSetMock).toHaveBeenCalledTimes(1);
        expect(logger.info).toHaveBeenCalledWith('Daily portfolio snapshots complete', expect.objectContaining({
    ...
    it('records errors when snapshot generation fails', async () => {
        ...
        await (capturedOnRun === null || capturedOnRun === void 0 ? void 0 : capturedOnRun());
        expect(logger.error).toHaveBeenCalledWith('Failed to generate snapshot', expect.objectContaining({
            userId: 'user-999',
...
//# sourceMappingURL=portfolioSnapshots.test.js.map