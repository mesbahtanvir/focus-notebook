...
exports.updateRecurringStreams = updateRecurringStreams;
exports.isKnownSubscriptionMerchant = isKnownSubscriptionMerchant;
const admin = __importStar(require("firebase-admin"));
const db = admin.firestore();
async function detectRecurringTransactions(uid, lookbackDays = 365) {
    var _a, _b;
    ...
    for (const doc of txnsSnapshot.docs) {
        const txn = doc.data();
        const merchant = ((_a = txn.merchant) === null || _a === void 0 ? void 0 : _a.normalized) || ((_b = txn.merchant) === null || _b === void 0 ? void 0 : _b.name) || 'Unknown';
...
function analyzeTransactionPattern(transactions) {
    if (transactions.length < 3) {
        return null;
    }
    // Sort by date
    ...
    const amountVariance = amounts.reduce((sum, val) => sum + Math.pow(val - meanAmount, 2), 0) / amounts.length;
...
// ============================================================================
async function updateRecurringStreams(uid, candidates) {
...
function isKnownSubscriptionMerchant(merchant) {
    const merchantLower = merchant.toLowerCase();
    return KNOWN_SUBSCRIPTION_MERCHANTS.some(known => merchantLower.includes(known));
}
