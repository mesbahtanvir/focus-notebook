...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const hideSidebar = pathname?.startsWith('/tools/photo-feedback/session')
    || pathname?.startsWith('/tools/photo-feedback/results')
    || pathname?.startsWith('/tools/photo-feedback/share');
  ...
  return (
    <div className="flex h-[100dvh] bg-gradient-to-br from-slate-50 via-white to-blue-50/50 bg-textured text-foreground" style={{
      ...
      {!hideSidebar && <Sidebar />}
      <main
        className={`flex-1 overflow-y-auto ${hideSidebar ? 'p-0' : 'p-8 md:p-12 lg:p-16 pt-12 lg:pt-8'}`}
        role="main"
      ...
      >
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={hideSidebar ? '' : 'max-w-6xl mx-auto'}
            id="main"
          >
            {children}
          </motion.div>
