...
describe('Task Store Integration', () => {
  ...
  describe('Recurring tasks', () => {
    ...
    it('should reset daily tasks', async () => {
      const tasks = [
        aTask().asDaily().asDone().build(),
        aTask().asDaily().asDone().build(),
        aTask().asWeekly().asDone().build(), // Should not be reset
      ];
      ...
      await waitFor(() => {
        ...
        const weeklyTask = allTasks.find(t => t.recurrence?.type === 'weekly');
        ...
        expect(weeklyTask?.done).toBe(true); // Unchanged
      });
    });
  });
