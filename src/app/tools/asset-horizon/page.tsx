...
export default function AssetHorizonPage() {
  ...
  const basePlanFromPortfolio = useMemo(() => {
    const planPortfolio = portfolios.find(portfolio => portfolio.recurringPlan);
    if (!planPortfolio?.recurringPlan) {
      return null;
    }
  ...
  useEffect(() => {
    ...
    const normalizedAmount = Number.isFinite(basePlanFromPortfolio.amount)
      ? Number(basePlanFromPortfolio.amount.toFixed(2))
      : 0;
    setBaseAmount(normalizedAmount);
    setBaseFrequency(basePlanFromPortfolio.frequency);
    if (
      Number.isFinite(basePlanFromPortfolio.expectedAnnualReturn) &&
      typeof basePlanFromPortfolio.expectedAnnualReturn === 'number'
    ) {
      setReturnRate(basePlanFromPortfolio.expectedAnnualReturn);
    }
  }, [basePlanFromPortfolio]);
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-2xl border border-rose-100 bg-white p-4 shadow-md">
                  ...
                  <p className="text-sm text-gray-500">Contribution fueling the journey</p>
                </div>
              </div>
            </div>
